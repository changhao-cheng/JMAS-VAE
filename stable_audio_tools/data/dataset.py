...
def read_audio_from_tar(path):
    ...
    # format:
    # (0) /path/to/audio_file.flac
    # (1) /path/to/audio_file.flac#[start1]:[end1],[start2]:[end2],...
    # (2) /path/to/tar_file.tar:[offset]:[size]
    # (3) /path/to/tar_file.tar:[offset]:[size]#[start1]:[end1],[start2]:[end2],...
    if ".tar" in path:
        assert ":" in path, path
        if "#" in path:
            ...
            start_end = None
        fileio = read_file_from_tar(tar_path, int(offset), int(size))
    else:
        if "#" in path:
            # read audio with start and end indices
            fileio, start_end = path.split("#")
            start_end = sorted(
                [tuple(map(int, x.split(":"))) for x in start_end.split(",")],
                key=lambda x: x[0],
            )
            ...
            fileio, start_end = path, None
    with sf.SoundFile(fileio) as f:
        sr = f.samplerate
        if start_end is None:
            ...
            for st, et in start_end:
                assert et > st, (st, et)
                f.seek(st)
                lst.append(f.read(et - st, dtype="float32"))
            wav = np.hstack(lst)
    return torch.from_numpy(wav).unsqueeze(0), sr


class SampleJsonDataset(torch.utils.data.Dataset):
    def __init__(
        self, 
        configs,
        sample_size=65536, 
        ...
        for config in configs:
            ...
            path_dir = Path(config.path).parent
            self.root_paths.append(str(path_dir))
            with open(config.path, 'r') as f:
                for dic in json.load(f):
                    self.filenames.append(dic["path"])
            if config.custom_metadata_fn is not None:
                self.custom_metadata_fns[config.path] = config.custom_metadata_fn
    ...
    def load_file(self, filename):
        audio, in_sr = read_audio_from_tar(filename)
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    if dataset_type in ("audio_dir", "audio_json"):
        ...
        elif dataset_type == "audio_json":
            train_set = SampleJsonDataset(
                configs,
                sample_size=sample_size,
                sample_rate=sample_rate,
                random_crop=dataset_config.get("random_crop", True),
