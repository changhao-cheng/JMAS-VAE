...
from ..models.bottleneck import VAEBottleneck, JMASVAESSL_new_Bottleneck
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
            ...
            latent_subspace: Optional[dict] = None,
            lr: float = 1e-4,
            warmup_steps: int = 0,
            ...
            adaptive_loss_back_weight: bool = False,
            adaptive_loss_keys: Optional[List[str]] = None,
            adaptive_loss_back_keys: Optional[List[str]] = None,
            reference_loss_key: Optional[str] = None,
            model_param_names: Optional[List[str]] = None,
            model_param_back_names: Optional[List[str]] = None
    ):
        super().__init__()
        ...
        self.adaptive_loss_back_weight = adaptive_loss_back_weight
        ...
        self.latent_subspace = latent_subspace
        if latent_subspace is not None:
            ls_enable = latent_subspace['enable']
            ssl_ratio = latent_subspace['ssl_ratio']
            self.ls_enable = ls_enable
        ...
        self.mcr_config = loss_config.get("mcr", None)
        self.use_mcr = self.mcr_config is not None
        self.mcr_spectral_loss = None

        # Spectral reconstruction loss
        self.use_spectral = 'spectral' in loss_config
        ...
        if self.use_mcr:
            mcr_cfg = self.mcr_config.get("config", {})
            self.mcr_spectral_loss = self.sdstft
            if self.mcr_spectral_loss is None and mcr_cfg.get("mrstft_weight", 1.0) > 0:
                mcr_stft_args = mcr_cfg.get(
                    "spectral_config",
                    {
                ...
                if self.autoencoder.out_channels == 2:
                    self.mcr_spectral_loss = auraloss.SumAndDifferenceSTFTLoss(
                        sample_rate=sample_rate,
                        **mcr_stft_args,
                    )
                else:
                    self.mcr_spectral_loss = auraloss.MultiResolutionSTFTLoss(
                        sample_rate=sample_rate,
                        **mcr_stft_args,
                    )
        ...
        if self.use_mcr and self.mcr_config.get("weights", {}).get("mcr", 0.0) > 0.0:
            self.gen_loss_modules.append(
                ValueLoss(
                    key="mcr",
                    weight=self.mcr_config["weights"]["mcr"],
                    name="mcr_loss",
                    decay=self.mcr_config.get("decay", 1.0),
                )
            )

        if self.adaptive_loss_weight and self.adaptive_loss_back_weight:
            self.losses_gen = MultiLoss(
                self.gen_loss_modules,
                adaptive_loss_keys=self.adaptive_loss_keys,
                adaptive_loss_back_keys=self.adaptive_loss_back_keys,
                reference_loss_key=self.reference_loss_key,
                model=self.autoencoder,
                model_param_names=self.model_param_names,
                model_param_back_names=self.model_param_back_names
            )
        elif self.adaptive_loss_weight and not self.adaptive_loss_back_weight:
            self.losses_gen = MultiLoss(
                self.gen_loss_modules,
                adaptive_loss_keys=self.adaptive_loss_keys,
                reference_loss_key=self.reference_loss_key,
    ...
    def _sample_mcr_direction(self, latents: torch.Tensor) -> torch.Tensor:
        cfg = self.mcr_config.get("config", {})
        direction = torch.randn_like(latents)
        if cfg.get("normalize_direction", True):
        ...
        if cfg.get("scale_by_latent_std", True):
            latent_scale = latents.detach().flatten(1).std(dim=1, unbiased=False).clamp_min(1e-6)
            direction = direction * latent_scale.view(-1, 1, 1)
        return direction

    def _mcr_pair_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        pred, target = trim_to_shortest(pred, target)
        cfg = self.mcr_config.get("config", {})
        loss = pred.new_tensor(0.0)
        l1_weight = float(cfg.get("l1_weight", 1.0))
        mrstft_weight = float(cfg.get("mrstft_weight", 1.0))

        if l1_weight > 0:
            loss = loss + l1_weight * torch.nn.functional.l1_loss(pred, target)
        if mrstft_weight > 0 and self.mcr_spectral_loss is not None:
            loss = loss + mrstft_weight * self.mcr_spectral_loss(target, pred)
        return loss

    def _compute_mcr_loss(self, latents: torch.Tensor, decoded_anchor: torch.Tensor) -> torch.Tensor:
        cfg = self.mcr_config.get("config", {})
        alpha_medium = float(cfg.get("alpha_medium", 0.01))
        alpha_large = float(cfg.get("alpha_large", 0.03))
        if alpha_large <= alpha_medium:
            raise ValueError("mcr.config.alpha_large must be greater than alpha_medium")

        delta = self._sample_mcr_direction(latents)
        z_medium = latents + alpha_medium * delta
        z_large = latents + alpha_large * delta

        decoded_medium = self.autoencoder.decode(z_medium)
        decoded_large = self.autoencoder.decode(z_large)

        loss_medium = self._mcr_pair_loss(decoded_medium, decoded_anchor.detach())
        loss_large = self._mcr_pair_loss(decoded_large, decoded_medium.detach())
        return loss_medium + loss_large
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_info.update(encoder_info)
        if self.ls_enable:
            total_dim = latents.shape[1]
            zA_dim = int(total_dim*self.ssl_ratio)
            zA = latents[:,:zA_dim, :]
            zB= latents[:,zA_dim:, :]
            zA_detached = zA.detach()
            latents = torch.cat([zA_detached, zB], dim=1)
        ...
        if use_disc:
            loss, losses, _ = self.losses_disc(loss_info)
        ...
        # Train the generator
            ...
            if self.use_mcr and self.mcr_config.get("weights", {}).get("mcr", 0.0) > 0.0:
                decoded_anchor_for_mcr = decoded_raw
                if self.latent_mask_ratio > 0.0:
                    decoded_anchor_for_mcr = self.autoencoder.decode(mcr_latents)
                loss_info["mcr"] = self._compute_mcr_loss(mcr_latents, decoded_anchor_for_mcr)

            loss, losses, adp_weights = self.losses_gen(loss_info)
            ...
            for key, value in adp_weights.items():
...
def create_loss_modules_from_bottleneck(bottleneck, loss_config):
    ...
    if isinstance(bottleneck, (VAEBottleneck, JMASVAESSL_new_Bottleneck)):
        try:
    ...
    if isinstance(bottleneck, JMASVAESSL_new_Bottleneck):
        try:
            ssl_weight = loss_config['bottleneck']['weights']['ssl']
        except:
            ssl_weight = 1e-3

        ssl_frame_loss = ValueLoss(key='ssl_1', weight=ssl_weight, name='ssl_frame_loss')
        losses.append(ssl_frame_loss)
        ssl_mat_loss = ValueLoss(key='ssl_2', weight=ssl_weight, name='ssl_mat_loss')
        losses.append(ssl_mat_loss)
